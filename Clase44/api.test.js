...
describe("Test over RESTFULL API", () => {
    ...
    describe("- POST /api/test", async () => {
        ...
        it("Response should have nombre, precio and thumbnail", () => {
            expect(createdProduct).to.keys("nombre", "precio", "thumbnail", "__v", "_id");
        });

        it("Should return created product", () => {
            expect(createdProduct.nombre).to.eql(product.nombre);
    ...
    describe("- GET /api/test", async () => {
        ...
        it("Response should have an array of objects with nombre, precio and thumbnail", () => {
            expect(Array.isArray(getProducts)).to.be.true;
            expect(getProducts.every(product => "nombre" in product && "precio" in product && "thumbnail" in product)).to.be.true;
        });
    });
